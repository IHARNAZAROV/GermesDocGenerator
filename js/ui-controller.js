...
// ── Тёмная тема ───────────────────────────────────────────────
const LS_THEME_KEY   = 'germesTheme';
...
function applyTheme(theme) {
  ...
  try { localStorage.setItem(LS_THEME_KEY, theme); } catch (_) {}
}
...
// data-theme всегда выставляется — либо из сохранённого выбора, либо из системной темы.
// Это позволяет CSS использовать только [data-theme="dark"] без @media-запросов.
(function initTheme() {
  const saved = localStorage.getItem(LS_THEME_KEY);
  applyTheme(saved || getSystemTheme());

  // Следим за изменением системной темы (только если нет ручного выбора)
